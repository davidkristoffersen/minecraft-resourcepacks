...
| [Camera](camera/) | The Camera plugin's camera as a cute instant camera - empty, loaded, flashing - and its photos as polaroids tinted with each picture's own colour. Repaints two vanilla item definitions only where the plugin sets `custom_model_data`; real compasses and maps are untouched. | [`Camera-1.2.0.zip`](camera/Camera-1.2.0.zip) |
| [ServerUI](serverui/) | Pixel icons at the emoji code points the server's dialog menus use, drawn into the default font. Without the pack the menus show the same symbols in Unifont. | [`ServerUI-1.10.1.zip`](serverui/ServerUI-1.10.1.zip) |
