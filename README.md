...
| [MobDesigner](mobdesigner/) | The Mob Designer's costumes - hood, coat, boots and face drawn as an armour layer over every designed zombie, husk, drowned and skeleton - and a picture for each design's spawn egg. Repaints the five egg item definitions only where the plugin sets `custom_model_data`. | [`MobDesigner-1.0.0.zip`](mobdesigner/MobDesigner-1.0.0.zip) |
| [ServerUI](serverui/) | Pixel icons at the emoji code points the server's dialog menus use, drawn into the default font. Without the pack the menus show the same symbols in Unifont. | [`ServerUI-1.13.0.zip`](serverui/ServerUI-1.13.0.zip) |
