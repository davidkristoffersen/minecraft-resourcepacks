# ThemeCalm

The main server's palette, for Minecraft Java 26.2+, **no mods**.

Download: [`ThemeCalm-1.0.1.zip`](ThemeCalm-1.0.1.zip)

Deliberately quiet: a warm gold hotbar frame, a slightly warmer sun, and a soft
amethyst chime for every menu click. Derived from the vanilla textures in the
installed client jar when the pack is built; the click is a remap of vanilla sound
events - no audio is shipped. `python3 build.py` builds; shared code lives in
`../themelib.py`.
